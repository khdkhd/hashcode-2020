...
function verify(file, weights) {
    const input = fs.readFileSync(file, charset);
    const lines = input.split(eol);
    lines.pop();
    const indices = chunkToNumbers(lines.pop());
    let score = 0;
...
function run() {
    ...
    const executionTimeInMillis = 1000 * 60 * 1;
    ...
    for (const instance of instances) {
        ...
        const outputFile = `${instance.file}.out`;
        dump(instance.solution.items, outputFile);
        const dumpedScore = verify(outputFile, instance.weights);
        console.log(`dumpedScore=${dumpedScore}`);
    }
}
